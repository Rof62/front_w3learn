    ...
    async function getDefaultImage() {
      ...
      if (user.blobby ) {
        response = await fetch(
          `https://back-w3learn.vercel.app/api/profileImage/getAvatarFromUser?id=${user.idUsers}`
        );
      } else {
        response = await fetch(
          `https://back-w3learn.vercel.app/api/profileImage/getDefaultImage`
        );
      }
      console.log(user);
  ...
  function handleSubmit(event) {
    ...
    fileReader.onload = async () => {
      ...
      const response = await fetch(
        "https://back-w3learn.vercel.app/api/profileImage/insertImage",
        {
          method: "PATCH",
